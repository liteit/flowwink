...
function hasNearbyStrip(lines: string[], violationIdx: number): boolean {
  ...
  // Inline opt-out: `// args-lint-ignore` on the violation line or up to 6 lines above
  const optOutWindow = lines.slice(Math.max(0, violationIdx - 6), violationIdx + 2).join('\n');
