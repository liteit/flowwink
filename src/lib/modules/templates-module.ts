...
const TEMPLATE_SKILLS: SkillSeed[] = [
  ...
  },
  {
    ...
    instructions:
      'STAGED SKILL (requires_staging=true): the first call does NOT install anything — it returns { staged: true, operation_id } with a preview. Handshake: 1) call install_template with your arguments, 2) call approve_pending_operation with p_id=<operation_id> (admin approval), 3) re-invoke install_template with the SAME arguments plus _approved_operation_id=<operation_id>. On execute it first uninstalls the previously installed template via its manifest (only resources that template created), then seeds content. Returns { created, skipped, manifest, uninstalled_previous, notes }. Colliding live slugs/names are skipped, never overwritten. Image URLs are used as-is (no media-library download). Site settings are untouched unless apply_settings=true.',
  },
  {
