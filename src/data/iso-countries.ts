 ...
 *
 * Names are resolved at display time via `Intl.DisplayNames` so we don't ship
 * a translated string table. Fallback to the code if the runtime can't
 * localize.
 */
...
/** Codes with resolved display names, sorted by name. */
export function listCountries(): Array<{ code: string; name: string }> {
  return ISO_COUNTRY_CODES.map((code) => ({ code, name: countryName(code) })).sort((a, b) =>
