...
interface Request {
  ...
  chain?: { name: string };
  step?: { required_role: string | null; min_approvals: number };
}
              ...
              <Card key={req.id}>
                <CardContent className="p-3 space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground ml-auto">{formatDistanceToNow(new Date(req.created_at), { addSuffix: true })}</span>
                  </div>
                  <Textarea
