...
DECLARE
  ...
  v_total int;
BEGIN
  -- Seed demo products if missing (always tag them so reset cleans them up)
  FOR prec IN
    SELECT * FROM (VALUES
  ...
  LOOP
    ...
    VALUES (p_run_id, 'products', v_prod_id);
  ...
  LOOP
    ...
    INSERT INTO public.demo_run_items (run_id, entity_table, entity_id)
    VALUES (p_run_id, 'orders', v_order_id);
    ...
    INSERT INTO public.demo_run_items (run_id, entity_table, entity_id)
    VALUES (p_run_id, 'order_items', v_item_id);
