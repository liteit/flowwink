-- B-round (2026-07-08): agent-administrability RPCs for parity features that
-- shipped UI-only. Applied to dev directly the same day; persisted here for the
-- fleet. All SECURITY DEFINER with the service_role escape (MCP gateway runs
-- RPC skills with the service key, so auth.uid() is NULL there).
